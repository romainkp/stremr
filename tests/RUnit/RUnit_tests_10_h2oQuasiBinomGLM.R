notest.h2oQuasiBinomGLM.Ensemble <- function() {
  ...
  # OData$H2Oframe[, "prev_Qkplus1"] <- OData$H2Oframe[, "Qkplus1"]
        ...
        # H2O glm w/ L_BFGS:
        params = list(fit.package = "h2o", fit.algorithm = "glm", solver = "L_BFGS", family = "quasibinomial")
        gcomp_est <- fitSeqGcomp(OData, tvals = t.surv, intervened_TRT = "gTI.dhigh", Qforms = Qforms, models = params,
        ...
        gcomp_est <- fitSeqGcomp(OData, tvals = t.surv, intervened_TRT = "gTI.dhigh", Qforms = Qforms, models = params, stratifyQ_by_rule = FALSE)
        gcomp_est$estimates[]
        #    est_name  t      risk      surv ALLsuccessTMLE nFailedUpdates rule.name
        ...
        gcomp_est <- fitSeqGcomp(OData, tvals = t.surv, intervened_TRT = "gTI.dhigh", Qforms = Qforms, models = SLparams, stratifyQ_by_rule = FALSE)
        gcomp_est$estimates[]

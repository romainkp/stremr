context("Fitting with no Monitoring and / or no Censoring indicators")
  ...
  # ## ----------------------------------------------------------------
  # ## Evaluate the weights for each regimen, separately.
  # ## ----------------------------------------------------------------
  # wts <- analysis %>%
  #     distinct(intervened_TRT) %>%
  #     mutate(IPWeights = map(intervened_TRT, getIPWeights, OData = OData)) %>%
  #     mutate(IPWeights = map(IPWeights, ~ .x[, "ID" := paste0(ID, rule.name)]))

  # IPWeights <- rbindlist(wts[["IPWeights"]])
  # setkeyv(IPWeights, c("ID", "t"))
  # attributes(IPWeights)[['intervened_TRT']] <- "A.star"
  # attributes(IPWeights)[['intervened_MONITOR']] <- NULL

  # ## ----------------------------------------------------------------
  # ## Stack the datasets into one for pooled TMLE/GCOMP
  # ## ----------------------------------------------------------------
  # Odat_DT_1 <- copy(Odat_DT)
  # Odat_DT_1[, "rule.name" := "gTI.dlow"][, "A.star" := gTI.dlow][, "ID" := paste0(ID, rule.name)]
  # Odat_DT_2 <- copy(Odat_DT)
  # Odat_DT_2[, "rule.name" := "gTI.dhigh"][, "A.star" := gTI.dhigh][, "ID" := paste0(ID, rule.name)]
  # Odat_DT_pooled <- rbindlist(list(Odat_DT_1, Odat_DT_2))
  # OData <- stremr::importData(Odat_DT_pooled, ID = "ID", t = "t", covars = c("highA1c", "lastNat1", "lastNat1.factor"), TRT = "TI", OUTCOME = "Y.tplus1")
  ...
  # ## ------------------------------------------------------------
  # delta <- 0.25
  # TMLE_maxpYdelta <- fit_TMLE(intervened_TRT = "A.star",
  #                              IPWeights = IPWeights,
  #                              stratifyQ_by_rule = TRUE,
  #                              tvals = tvals,
  #                               OData = OData,

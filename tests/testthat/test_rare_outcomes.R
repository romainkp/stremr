...
#   ## ----------------------------------------------------------------
#   delta <- mean(Odat_DT[, Y.tplus1], na.rm = TRUE)
#   delta <- 1
#   Odat_DT[t == tvals,]
#   Odat_DT[, "Y.tplus1.star" := as.numeric(Y.tplus1)]
#   Odat_DT[t == tvals, "Y.tplus1.star" := Y.tplus1 / delta]
#   Odat_DT[, Y.tplus1.star]

#   OData <- stremr::importData(Odat_DT, ID = "ID", t = "t", covars = c("highA1c", "lastNat1", "lastNat1.factor"), TRT = "TI", OUTCOME = "Y.tplus1.star")

...
SLfit.h2ogrid <- function(grid.algorithm, training_frame, y, x, family = "binomial", fold_column, model_contrl, ...) {
  ...
  if (is.null(mainArgs$hyper_params)) stop("must specify hyper parameters for grid search by defining a SuperLearner params list item named '" %+% grid.algorithm %+% "'")
...
fit.h2oSuperLearner <- function(fit.class, fit, training_frame, y, x, model_contrl, fold_column, ...) {
  ...
  # browser()
  # length(fitted_models_all)
  # fitted_models_all2 <- fitted_models_all[1:5]
  # fitted_models_all2[[1]]@model$cross_validation_predictions[[2]]
  # fitted_models_all2[[2]]@allparameters$nfolds

  # Specify a defalt GLM as the metalearner
  metalearner <- model_contrl$metalearner
  if (is.null(metalearner)) metalearner <- "h2o.glm_nn"
  stack <- h2oEnsemble::h2o.stack(models = fitted_models_all, response_frame = training_frame[,y], metalearner = metalearner)

  # ----------------------------------------------------------------------------------------------------
  # Compute the final SL performance on the training set:
  # ----------------------------------------------------------------------------------------------------
  print("SuperLearner fit:"); print(stack$metafit)
  perf <- h2oEnsemble::h2o.ensemble_performance(stack, newdata = training_frame, score_base_models = FALSE)
  ...
  # perf3 <- h2o.ensemble_performance(stack3, newdata = training_frame, score_base_models = FALSE)
  # print(perf3)
